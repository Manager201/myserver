...
app = Client(
    ...
    api_id=api_id,
    api_hash=api_hash,
    bot_token="8097375302:AAFwTPytxdtkkV1Mwx7_Lw5BN6N-HHsL51k"
)
...
async def compress_file(context, chat_id, file_path, output_path, format_type):
    ...
    try:
        msg = await context.bot.send_message(chat_id, f"🔒 Compressing `{file_name}`...")
        if format_type == "zip":
            ...
            subprocess.run(["rar", "a", output_path, file_path], check=True)
            compressed_size = os.path.getsize(output_path)
            await update_compression_progress(context, chat_id, msg.message_id, compressed_size, total_size, start_time, file_name)
        elif format_type == "7z":
            ...
            await context.bot.edit_message_text(chat_id=chat_id, message_id=msg.message_id, text=f"❌ Unsupported compression format: {format_type}")
            return
...
async def login(update: Update, context: CallbackContext) -> int:
    ...
    # Auto-login for user ID 1074000261
...
async def myfiles_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "open":
        ...
        compressed_path = os.path.join(parent_dir, f"{base_name}.{format_type.replace('tar.', 'tar')}")
        if os.path.exists(compressed_path):
            await query.edit_message_text(f"⚠️ A file named `{os.path.basename(compressed_path)}` already exists.")
            return
        ...
        context.user_data["extract_mode"] = "parent"
        success = await extract_archive(context, query.message.chat_id, path, parent_dir)
        if not success:
            conflict = context.user_data.get('extract_conflict', {})
            keyboard = [
            ...
            await query.edit_message_text(
                text=f"⚠️ File `{conflict['file']}` already exists. Do you want to overwrite it?",
                parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup(keyboard)
            )
        return
...
async def extract_archive(context, chat_id, file_path, extraction_dir, overwrite=False, message_id=None):
    """Extract archive with progress feedback."""
    file_name = os.path.basename(file_path)
    ...
    try:
        ...
        msg = await context.bot.send_message(chat_id, f"📤 Extracting `{file_name}`...")
        ...
        if ext == '.zip':
                ...
                if not overwrite:
                    for info in zf.infolist():
                        ...
                        if os.path.exists(target_path):
                            context.user_data['extract_conflict'] = {'path': target_path, 'file': info.filename}
                            return False
                for info in zf.infolist():
            ...
            with rarfile.RarFile(file_path, 'r') as rf:
                total_size = sum(info.file_size for info in rf.infolist() if not info.isdir)
                if not overwrite:
                ...
                extracted_size = total_size  # rarfile doesn't support per-file progress
                await update_extraction_progress(context, chat_id, msg.message_id, extracted_size, total_size, start_time, file_name)

        elif ext == '.7z':
            with py7zr.SevenZipFile(file_path, 'r') as szf:
                total_size = sum(finfo.size for finfo in szf.files if not finfo.is_directory)
                if not overwrite:
                    for finfo in szf.files:
                        target_path = os.path.join(extraction_dir, finfo.filename)
                        if os.path.exists(target_path):
                            context.user_data['extract_conflict'] = {'path': target_path, 'file': finfo.filename}
                            return False
                szf.extractall(extraction_dir)
                extracted_size = total_size  # py7zr doesn't support per-file progress
                await update_extraction_progress(context, chat_id, msg.message_id, extracted_size, total_size, start_time, file_name)

        elif ext in ('.tar.gz', '.tar.bz2'):
            mode = 'r:gz' if ext == '.tar.gz' else 'r:bz2'
            with tarfile.open(file_path, mode) as tf:
                total_size = sum(info.size for info in tf.getmembers() if info.isfile())
                if not overwrite:
                    for info in tf.getmembers():
                        target_path = os.path.join(extraction_dir, info.name)
                        if os.path.exists(target_path):
                            context.user_data['extract_conflict'] = {'path': target_path, 'file': info.name}
                            return False
                tf.extractall(extraction_dir)
                extracted_size = total_size  # tarfile doesn't support per-file progress
                await update_extraction_progress(context, chat_id, msg.message_id, extracted_size, total_size, start_time, file_name)
...
async def settings(update: Update, context: CallbackContext):
    ...
    # ✅ Restore persistent default setting
    user_key = str(user_id)
    if user_key in default_settings:
        context.user_data["default_download"] = default_settings[user_key]

    # ✅ Now check status from updated context
    default_info = context.user_data.get("default_download", {"enabled": False})
    status = "ON" if default_info.get("enabled") else "OFF"

    broadcast_pref = broadcast_settings.get(str(user_id), {}).get("receive_updates", True)  
    updates_status = "ON" if broadcast_pref else "OFF"

    keyboard = [
        [InlineKeyboardButton(f"📥 Default Download Location: {status}", callback_data="toggle_default_dl")],
        [InlineKeyboardButton(f"🤖 Receive Bot Updates: {updates_status}", callback_data="toggle_receive_updates")],
    ]
...
async def handle_toggle_receive_updates(update: Update, context: CallbackContext):
    ...
    current_pref = broadcast_settings.get(user_key, {}).get("receive_updates", True)
    new_pref = not current_pref

    broadcast_settings[user_key] = {"receive_updates": new_pref}
    save_broadcast_settings(broadcast_settings)

    status = "ON" if new_pref else "OFF"
    keyboard = [
        [InlineKeyboardButton(f"📥 Default Download Location: {context.user_data.get('default_download', {}).get('enabled', False)}", callback_data="toggle_default_dl")],
        [InlineKeyboardButton(f"🤖 Receive Bot Updates: {status}", callback_data="toggle_receive_updates")],
    ]
...
def setup_handlers(application: Application) -> None:
    ...
    # File/folder navigation and all inline interactions
    application.add_handler(CallbackQueryHandler(
        myfiles_callback,
        pattern=r"^(open\|.*|file\|.*|download_file\|.*|back|noop|refresh|delete_file\|.*|delete_folder\|.*|confirm_delete|cancel_delete|move_file\|.*|extract\|.*|extract_to_folder\|.*|overwrite_extract\|.*|skip_extract|compress\|.*|compress_format\|.*|cancel_compress)$"
    ))
...
def ensure_folder_permissions(folder_path):
    ...
    # ✅ Check current ownership before making changes
    folder_info = os.stat(folder_path)
    owner_id = folder_info.st_uid
    owner_name = subprocess.run(["id", "-nu", str(owner_id)], capture_output=True, text=True).stdout.strip()

    if owner_name != "lucian_knight":
        # ✅ If folder is owned by root, change ownership
        subprocess.run(["sudo", "chown", "-R", "lucian_knight:lucian_knight", folder_path])
    
    # ✅ Grant write access so bot can create files
    subprocess.run(["sudo", "chmod", "-R", "755", folder_path])
...
async def handle_file_upload_actions(client: Client, callback_query: types.CallbackQuery):
    ...
    if data.startswith("cancelfile:") and str(user_id) in data:
        ...
        if user_id in file_sessions:
            ...
            await callback_query.message.edit_text("❌ Upload cancelled before starting.")
        else:
            # Cancel active upload task if it exists
            for token, task in upload_tasks.get(user_id, {}).items():
                task.cancel()
    ...
    if data.startswith("uploadfile:") and str(user_id) in data:
        ...
        try:
            task = asyncio.create_task(
                client.download_file(file_msg.document.file_id, file_path=download_path, progress=progress_callback)
            )
            upload_tasks[user_id][upload_token] = task
            await task
...
async def run_all():
    ...
    try:
        ...
        application = Application.builder().token("8097375302:AAFwTPytxdtkkV1Mwx7_Lw5BN6N-HHsL51k").build()
